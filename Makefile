...
migrateup1:
	migrate -path db/migration -database "postgresql://root:secret@localhost:5411/simple_bank?sslmode=disable" --verbose up 1

migratedown:
	migrate -path db/migration -database "postgresql://root:secret@localhost:5411/simple_bank?sslmode=disable" --verbose down

migratedown1:
	migrate -path db/migration -database "postgresql://root:secret@localhost:5411/simple_bank?sslmode=disable" --verbose down 1
